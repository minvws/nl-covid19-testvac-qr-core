...
func OkTest() *Result {
	v, _ := json.Marshal([]byte("{1: 2}"))
	return &Result{Value: v, Error: ""}
}

func ErrorTest() *Result {
	return &Result{Value: nil, Error: "This is an error"}
}
...
func CreateCommitmentMessage(holderSkJson, issuerPkXml, issuerNonceJson []byte) []byte {
...
func CreateCredential(holderSkJson, ccmJson []byte) []byte {
	var holderSk *big.Int
	err := json.Unmarshal(holderSkJson, holderSk)
	if err != nil {
		return nil//, errors.WrapPrefix(err, "Could not unmarshal holder sk", 0).Error()
...
func DiscloseAllWithTime(credJson []byte) []byte {
